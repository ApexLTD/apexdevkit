...
def test_should_raise_on_bad_request() -> None:
    with pytest.raises(FakeHttpError):
        FakeResponse(status_code=400).on_bad_request(raises=FakeHttpError)


def test_should_raise_on_conflict() -> None:
    with pytest.raises(FakeHttpError):
        FakeResponse(status_code=409).on_conflict(raises=FakeHttpError)


def test_should_raise_on_server_error() -> None:
    with pytest.raises(FakeHttpError) as cm:
        FakeResponse(status_code=500).on_failure(raises=FakeHttpError)
...
def test_should_raise_on_not_found() -> None:
    with pytest.raises(FakeHttpError):
        FakeResponse(status_code=404).on_not_found(raises=FakeHttpError)
